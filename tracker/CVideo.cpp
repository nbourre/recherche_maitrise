...
void CVideo::PlayProcessed()
{
	...
	while(1)
	{
		...
		//////////////////////////////////////////////////////////////
		// Fin du code de projection horizontale
		//////////////////////////////////////////////////////////////
		...
		if (vecPeakHor.size() >= runningAverageSize)
		{
			...
			// Créer la projection verticale
			projV.CreateVerticalProjection(iplDifference);		

			// Lisse la projection verticale
			projV.Smooth(kernelSize);

			////////////////////////////////////////////////////			
			// Gestion des Maximums dans la projection verticale
			////////////////////////////////////////////////////
			...
			// Premier max
			firstPeak = projV.MaxPeak;

			
			// Trouve le prochain max selon les conditions établies
			if (loopCounter < (runningAverageSize + runningAverageSize))
			{
				secondPeak = projV.FindNextPeak();
			}
			// Si le nombre de distance est suffisant on peut seuiller.
			else
			{
				avgDist = (int)calcAverage(vecDistance);

				secondPeak = projV.FindNextPeak(avgDist, seuilMax);
			}

			vecDistance.push_back(abs(firstPeak.Position - secondPeak.Position));


			// Génère l'image de la projection verticale
			iplProjectionV = projV.CreateProjectionImage();

			
			if (firstPeak.Position > 0 && secondPeak.Position > 0)
			{
				// Dessine les lignes des maximums verticaux
				cvLine(iplT, cvPoint(firstPeak.Position, 0), cvPoint(firstPeak.Position, iplT->height), CV_RGB(255, 255, 255));
				cvLine(iplT, cvPoint(secondPeak.Position, 0), cvPoint(secondPeak.Position, iplT->height), CV_RGB(255, 255, 255));
			}
		}
...
//void CVideo::DrawCrossPoint(IplImage *img, vector <CvPoint> crossPoints)
//{
//	for (int i = 0; i < crossPoints.capacity(); i++)
//	{
//		cvCircle(img, crossPoints.at(i), 5, CV_RGB(255, 255, 255), 1, 8, 0);
//	}
//}
...
/// Ajoute au vecteur de int un valeur
void CVideo::addToVector(vector <int> *vec, int value)
{
	int actualSize = vec->size();
	vector <int>::iterator firstElem;
...
///  Retourne la moyenne d'un vecteur de int
double CVideo::calcAverage(vector <int> vec)
{
	double avg = 0;
	int i;
	int nbElements = vec.size();
